...
def build_clip_command(
    *,
    ...
    vf = _video_filter_chain(width=w, height=h, fps=config.target_fps, is_hdr=is_hdr)
    args: list[str] = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
    ...
    args += [
        ...
        "libx264",
        "-preset",
        "ultrafast",  # intermediate; mezzanine uses config.video_preset
        "-crf",
        "18",
        "-pix_fmt",
        "yuv420p",
...
async def extract_clips(
    ...
    async def _one(position: int, scene_idx: int) -> ClipInfo:
        ...
        # Apply trim offsets to the first and last clip of the reel.
        if position == 0:
